...
Shopify.onCartUpdate = function(cart) {
  Shopify.cartUpdateInfo(cart, '.cart-list');
};
      ...
      jQuery.each(cart, function(key, value) {
        if (key === 'items') {
          ...
          if (value.length) {
            ...
            jQuery(".empty_cart_area").css({"display": "none"});
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<li  class="single-cart-item-loop"><div class="cart-img"><a href="' + item.url + '"><img src="' + item.image + '" alt=""></a></div><div class="cart-info"><h6 class="product-name"><a href="' + item.url + '">' + item.title + '</a></h6><span class="cart-qty">'  +  item.quantity+'x' + '</span><span class="item-price">' + Shopify.formatMoney(item.price, formatMoney) + '</span></div><div class="del-icon"><a href="javascript:void(0);" onclick="Shopify.removeItem(' + item.variant_id + ')"> <i class="fa fa-times"></i></a></div></li>').appendTo(table);
            });
          }
          else {
            jQuery(".minicart-content-box").css({"display": "none"});
            jQuery(".empty_cart_area").css({"display": "block"});
          }
        }
      });
