...
lmp_update_state=function(){current_style()}})})(jQuery);function lmp_execute_func(func){if(the_lmp_js_data.javascript!='undefined'&&the_lmp_js_data.javascript!=null&&typeof func!='undefined'&&func.length>0){try{eval(func)}catch(err){alert('You have some incorrect JavaScript code (Load More Products)')}}}
...
return!1},"Please specify a valid CIF number.");$.validator.addMethod("cpfBR",function(value){value=value.replace(/([~!@#$%^&*()_+=`{}\[\]\-|\\:;'<>,.\/? ])+/g,"");if(value.length!==11){return!1}
