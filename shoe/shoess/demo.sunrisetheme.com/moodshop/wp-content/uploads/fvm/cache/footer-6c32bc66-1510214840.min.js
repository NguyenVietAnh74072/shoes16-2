...
function init(){var container=document.getElementById('st-container'),reset=document.getElementById('closeMenu'),buttons=Array.prototype.slice.call(document.querySelectorAll('.st-trigger')),eventtype=mobilecheck()?'touchstart':'click',resetMenu=function(){classie.remove(container,'st-menu-open')},bodyClickFn=function(evt){if(!hasParentClass(evt.target,'st-menu')&&!hasParentClass(evt.target,'offcanvas')){resetMenu();document.removeEventListener(eventtype,bodyClickFn)}},resetClickFn=function(evt){if(evt.target==reset){resetMenu();document.removeEventListener(eventtype,bodyClickFn)}};buttons.forEach(function(el,i){var effect=el.getAttribute('data-effect');el.addEventListener(eventtype,function(ev){ev.stopPropagation();ev.preventDefault();container.className='st-container';classie.add(container,effect);setTimeout(function(){classie.add(container,'st-menu-open')},25);document.addEventListener(eventtype,bodyClickFn);document.addEventListener(eventtype,resetClickFn)})})}
init()})()
...
;/*! Stellar.js v0.6.2 | Copyright 2014, Mark Dalgleish | //markdalgleish.com/projects/stellar.js | //markdalgleish.mit-license.org */
...
;(function($){"use strict";$(function(){$('.woocommerce div.quantity').append('<div class="inc c-btn"><i class="fa fa-caret-up"></i></div><div class="dec c-btn"><i class="fa fa-caret-down"></i></div>');$(".c-btn").on("click",function(){var $button=$(this);var oldValue=$button.parent().find("input").val();if($button.hasClass('inc')){var newVal=parseFloat(oldValue)+1}else{if(oldValue>1){var newVal=parseFloat(oldValue)-1}else{newVal=1}}
$button.parent().find("input").val(newVal).trigger("change")})});$('ul.products > li.product').each(function(){var $this=$(this),$height=$this.find('img.attachment-shop_catalog').height();$this.find('.basr-shop-extras').css('top',$height/2)});$(document).ready(function(){$('#primary .woof_sid_auto_shortcode ').on('click',function(){$(this).find('.woof_redraw_zone').on('click',function(e){e.stopPropagation()}).slideToggle()});var $fake_coupon=$('body.woocommerce-cart .cart-collaterals .coupon'),$coupon=$('body.woocommerce-cart .actions .coupon');if($fake_coupon.length>0){$fake_coupon.find('input[type="text"]').on('change',function(){$coupon.find('input[type="text"]').val($(this).val())});$fake_coupon.find('.button').on('click',function(){$coupon.find('.button').trigger('click')})}
if($('form.cart .tm-variation-ul-radio').length>0){$('form.cart .tm-variation-ul-radio').each(function(){var $table=$(this);var $label=$('form.cart .tm-variation-ul-radio').closest('.cpf-type-variations').find('.tm-epo-field-label');$label.addClass('ms-not-active ms-radio');$label.on('click',function(){$table.slideToggle();$(this).toggleClass('ms-not-active')})})}
var anm=!0;$('.wrap-shop-top-bar .basr-switch-shop-layout span').on('click',function(){var $order=$('.woocommerce-ordering .orderby');var order=$(this).attr('class');var $products=$(this).closest('.wrap-shop-top-bar').next();if(anm){anm=!1;$products.fadeOut(0).delay(10).fadeIn(1500);$products.addClass('loading');setTimeout(function(){anm=!0;$products.removeClass('loading')},1500);$(this).parent().find('.active').removeClass('active');$(this).addClass('active');if($(this).hasClass('listing')){$('body').addClass('shop-listing-style')}else{$('body').removeClass('shop-listing-style');var data_cols=$(this).attr('data-cols');if(data_cols){$('body').addClass(data_cols)}}}});$('.style-1 .summary .gallery-top .td-slick').each(function(){$(this).on('setPosition',function(event,slick,currentSlide){var slick_obj=$(this).slick('getSlick')
var height=slick_obj.listHeight+30+76+80-21;$(this).closest('.summary').css('min-height',height+'px')})});$('.nav-gallery  .slick-slide').on('click',function(){var index=$(this).attr('data-slick-index');$(this).closest(".gallery-thumbs").prev().find(".main-gallery").slick("slickGoTo",index)});var $swipe_trigger=$('.basr-trigger-swipe'),$gallery_top=$('.gallery-top');$gallery_top.find('.main-gallery a').on('click',function(e){e.preventDefault();$swipe_trigger.trigger('click')});$('.basr-gallery-ctn a').on('click',function(e){e.preventDefault();$swipe_trigger.trigger('click')})
$gallery_top.find('figure').on('click',function(){$swipe_trigger.trigger('click')})
$swipe_trigger.on('click',function(e){e.preventDefault();var pswpElement=$('.pswp')[0],items=[],eventTarget=$(e.target),clicked;items=$.parseJSON($(this).attr('data-swipe'));clicked=$('.gallery-top .slick-slider').slick('slickCurrentSlide');var options={index:clicked,shareEl:!1,closeOnScroll:!1,history:!1,hideAnimationDuration:0,showAnimationDuration:0};var photoswipe=new PhotoSwipe(pswpElement,PhotoSwipeUI_Default,items,options);photoswipe.init()})})})(jQuery)
;(function($){"use strict";$('.related .products').slick({slide:'li',slidesToShow:4,slidesToScroll:1,dots:!1,arrows:!1,adaptiveHeight:!0,responsive:[{breakpoint:1025,settings:{slidesToShow:3,slidesToScroll:1,}},{breakpoint:769,settings:{slidesToShow:2,slidesToScroll:1}},{breakpoint:480,settings:{slidesToShow:1,slidesToScroll:1}}]});if($('body.woocommerce').hasClass('style-2')){$('.tm-variation-ul-color .tmhexcolorimage-li-nowh').on('click',function(){$('.basr-gallery-ctn').slick('slickGoTo',0)})
var $gallery=$('body.style-2 .basr-gallery-ctn');var gallery_slick=function(){$gallery.slick({slidesToShow:2,slidesToScroll:1,dots:!1,arrows:!1,adaptiveHeight:!0,responsive:[{breakpoint:480,settings:{slidesToShow:1,slidesToScroll:1}},]})};if(window.innerWidth<=768){gallery_slick()}
$(window).resize(function(){if(window.innerWidth<=768){if(!$gallery.hasClass('slick-slider')){gallery_slick()}}else{if($gallery.hasClass('slick-slider')){$gallery.slick('unslick')}}})}})(jQuery)
;function hasParentClass(e,classname){if(e===document)return!1;if(classie.has(e,classname)){return!0}
return e.parentNode&&hasParentClass(e.parentNode,classname)}
function mobilecheck(){var check=!1;(function(a){if(/(android|ipad|playbook|silk|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(a)||/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0,4)))check=!0})(navigator.userAgent||navigator.vendor||window.opera);return check}(function($){var $header=$('#masthead');var $page=$('#page');if($header.hasClass('header-fixed')||$header.hasClass('headroom')){var $header_h=$header.height();$page.css('padding-top',$header_h+'px')}
var $cartTrigger=$('.header-builder .header-builder-type-header-buttons .cart');$cartTrigger.on('click',function(e){$cartTrigger.toggleClass('active');$cartTrigger.find('.widget_shopping_cart').fadeToggle();return!1});$cartTrigger.find('.widget_shopping_cart').on('click',function(e){e.stopPropagation()});if($("#lucy-auto-m-logo").length){var _logo=$('.header-builder-type-logo:first').find('a').html();$('#lucy-auto-m-logo').html(_logo)}})(jQuery)
...
function woof_submit_link(link){woof_show_info_popup(woof_lang_loading);if(woof_is_ajax===1&&!woof_ajax_redraw){woof_ajax_first_done=!0;var data={action:"woof_draw_products",link:link,page:woof_ajax_page_num,shortcode:jQuery('#woof_results_by_ajax').data('shortcode'),woof_shortcode:jQuery('div.woof').data('shortcode')};jQuery.post(woof_ajaxurl,data,function(content){content=jQuery.parseJSON(content);if(jQuery('.woof_results_by_ajax_shortcode').length){jQuery('#woof_results_by_ajax').replaceWith(content.products)}else{jQuery('.woof_shortcode_output').replaceWith(content.products)}
jQuery('div.woof_redraw_zone').replaceWith(jQuery(content.form).find('.woof_redraw_zone'));woof_remove_empty_elements();woof_init_search_form();woof_hide_info_popup();woof_draw_products_top_panel();woof_init_beauty_scroll();woof_init_ion_sliders();woof_reinit_native_woo_price_filter();jQuery.each(jQuery('#woof_results_by_ajax'),function(index,item){if(index==0){return}
jQuery(item).removeAttr('id')});woof_js_after_ajax_done()})}else{if(woof_ajax_redraw){var data={action:"woof_draw_products",link:link,page:1,shortcode:'woof_nothing',woof_shortcode:jQuery('div.woof').eq(0).data('shortcode')};jQuery.post(woof_ajaxurl,data,function(content){content=jQuery.parseJSON(content);jQuery('div.woof_redraw_zone').replaceWith(jQuery(content.form).find('.woof_redraw_zone'));woof_remove_empty_elements();woof_init_search_form();woof_hide_info_popup();woof_init_beauty_scroll();woof_init_ion_sliders();woof_reinit_native_woo_price_filter()})}else{window.location=link}}}
...
var txt=v;if(index=='orderby'){if(woof_lang[v]!==undefined){txt=woof_lang.orderby+': '+woof_lang[v]}else{txt=woof_lang.orderby+': '+v}}else if(index=='perpage'){txt=woof_lang.perpage}else if(index=='price'){txt=woof_lang.pricerange}else{var is_in_custom=!1;if(Object.keys(woof_lang_custom).length>0){jQuery.each(woof_lang_custom,function(i,v){if(i==index){is_in_custom=!0;txt=v}})}
...
panel.find('ul li a').click(function(){var tax=jQuery(this).data('tax');var name=jQuery(this).attr('href');if(tax!='price'){values=woof_current_values[tax];values=values.split(',');var tmp=[];jQuery.each(values,function(index,value){if(value!=name){tmp.push(value)}});values=tmp;if(values.length){woof_current_values[tax]=values.join(',')}else{delete woof_current_values[tax]}}else{delete woof_current_values.min_price;delete woof_current_values.max_price}
...
function woof_init_beauty_scroll(){if(woof_use_beauty_scroll){try{jQuery(".woof_section_scrolled").mCustomScrollbar('destroy');jQuery(".woof_section_scrolled").mCustomScrollbar({scrollButtons:{enable:!0},advanced:{updateOnContentResize:!0,updateOnBrowserResize:!0},theme:"dark-2",horizontalScroll:!1,mouseWheel:!0,scrollType:'pixels',contentTouchScroll:!0})}catch(e){console.log(e)}}}
...
function woof_checkboxes_slide(){if(woof_checkboxes_slide_flag==!0){var childs=jQuery('ul.woof_childs_list');if(childs.size()){jQuery.each(childs,function(index,ul){var span_class='woof_is_closed';if(jQuery(ul).find('input[type=checkbox],input[type=radio]').is(':checked')){jQuery(ul).show();span_class='woof_is_opened'}
...
;function woof_init_radios(){if(icheck_skin!='none'){jQuery('.woof_radio_term').iCheck('destroy');jQuery('.woof_radio_term').iCheck({radioClass:'iradio_'+icheck_skin.skin+'-'+icheck_skin.color,});jQuery('.woof_radio_term').on('ifChecked',function(event){jQuery(this).attr("checked",!0);var slug=jQuery(this).data('slug');var name=jQuery(this).attr('name');var term_id=jQuery(this).data('term-id');woof_radio_direct_search(term_id,name,slug)})}else{jQuery('.woof_radio_term').on('change',function(event){jQuery(this).attr("checked",!0);var slug=jQuery(this).data('slug');var name=jQuery(this).attr('name');var term_id=jQuery(this).data('term-id');woof_radio_direct_search(term_id,name,slug)})}
jQuery('.woof_radio_term_reset').click(function(){woof_radio_direct_search(jQuery(this).data('term-id'),jQuery(this).attr('name'),0);return!1})}
function woof_radio_direct_search(term_id,name,slug){jQuery.each(woof_current_values,function(index,value){if(index==name){delete woof_current_values[name];return}});if(slug!=0){woof_current_values[name]=slug;jQuery('a.woof_radio_term_reset_'+term_id).hide();jQuery('woof_radio_term_'+term_id).filter(':checked').parents('li').find('a.woof_radio_term_reset').show();jQuery('woof_radio_term_'+term_id).parents('ul.woof_list_radio').find('label').css({'fontWeight':'normal'});jQuery('woof_radio_term_'+term_id).filter(':checked').parents('li').find('label.woof_radio_label_'+slug).css({'fontWeight':'bold'})}else{jQuery('a.woof_radio_term_reset_'+term_id).hide();jQuery('woof_radio_term_'+term_id).attr('checked',!1);jQuery('woof_radio_term_'+term_id).parent().removeClass('checked');jQuery('woof_radio_term_'+term_id).parents('ul.woof_list_radio').find('label').css({'fontWeight':'normal'})}
woof_ajax_page_num=1;if(woof_autosubmit){woof_submit_link(woof_get_submit_link())}}
;function woof_init_checkboxes(){if(icheck_skin!='none'){jQuery('.woof_checkbox_term').iCheck('destroy');jQuery('.woof_checkbox_term').iCheck({checkboxClass:'icheckbox_'+icheck_skin.skin+'-'+icheck_skin.color,});jQuery('.woof_checkbox_term').on('ifChecked',function(event){jQuery(this).attr("checked",!0);woof_checkbox_process_data(this,!0)});jQuery('.woof_checkbox_term').on('ifUnchecked',function(event){jQuery(this).attr("checked",!1);woof_checkbox_process_data(this,!1)})}else{jQuery('.woof_checkbox_term').on('change',function(event){if(jQuery(this).is(':checked')){jQuery(this).attr("checked",!0);woof_checkbox_process_data(this,!0)}else{jQuery(this).attr("checked",!1);woof_checkbox_process_data(this,!1)}})}}
...
;(function($){"use strict";var BASR=BASR||{};var isMobile={Android:function(){return navigator.userAgent.match(/Android/i)},BlackBerry:function(){return navigator.userAgent.match(/BlackBerry/i)},iOS:function(){return navigator.userAgent.match(/iPhone|iPad|iPod/i)},Opera:function(){return navigator.userAgent.match(/Opera Mini/i)},Windows:function(){return navigator.userAgent.match(/IEMobile/i)},any:function(){return(isMobile.Android()||isMobile.BlackBerry()||isMobile.iOS()||isMobile.Opera()||isMobile.Windows())}};BASR.animated=function(){$('*[data-animation-enable="1"]').each(function(){var $this=$(this);var delay=$this.data('animation-delay');var animation=$this.data('animation');$this.one('inview',function(event,isInView,visiblePartX,visiblePartY){setTimeout(function(){$this.addClass(animation+' animated ')},delay)})})}
BASR.isotope=function(){if($().isotope&&$().imagesLoaded){if($('.basr-isotope-filter').length>0){$('.basr-isotope-filter .menu a').on('click',function(){$('.basr-isotope-filter .active').removeClass('active');$(this).parent().addClass('active');var selector=$(this).attr('data-filter');var $container=$(this).closest('.basr-page-top-menu').siblings().find('.grid-layout');console.log($container);$container.isotope({filter:selector,});return!1})}
$('.basr-isotope-wrapper').each(function(){var $this=$(this);if($this.find('ul.products').length>0){var $container=$this.find('.products')}else{var $container=$this.find('.basr-isotope-container')}
$this.imagesLoaded(function(){var selector='.grid-item';if($container.hasClass('products'))selector='.product';var isotope_args={itemSelector:selector,transitionDuration:'.55s',masonry:{gutter:0,},};if($this.hasClass('layout-no-padding')){isotope_args.masonry.columnWidth='.grid-size'}else if($this.hasClass('layout-free')){isotope_args.masonry.columnWidth='.width-1'}else{isotope_args.layoutMode='fitRows'}
var wait_vc=0;$container.isotope(isotope_args);var $banner=$container.find('.moodshop-banner');if($banner.next().length>0){$banner.css('min-height',$banner.next().outerHeight()+'px')}else if($banner.prev().length>0){$banner.css('min-height',$banner.prev().outerHeight()+'px')}
var $filter=$container.parent().parent().find('.basr-isotope-filter');if($filter.length>0){var $filter_active=$filter.find('.active');$filter.find('span').on('click',function(){$filter.find('.active').removeClass('active');$(this).addClass('active');$container.isotope({filter:'.'+$(this).attr('data-filter')});var $slick=$('.'+$(this).attr('data-filter')).find('.slick-slider');if($slick.length>0){$slick.slick('unslick');$slick.slick()}
$container.isotope({filter:'.'+$(this).attr('data-filter')})});if($filter_active.length>0){$container.isotope({filter:'.'+$filter_active.attr('data-filter')})}else{var $filter_active=$filter.find('span:first-child');if($filter_active){$filter_active.addClass('active');$container.isotope({filter:'.'+$filter_active.attr('data-filter')})}}}})});if($('.wc-listing-filter').length>0){$('.wc-listing-filter .product-cat').on('click',function(){var $container=$(this).closest('.basr-wc_listing');var $isotope=$container.find('.products');var sc=$(this).attr('data-content');var $items=$isotope.find('.product');$container.addClass('load-product');if($items.length>0){$isotope.isotope('remove',$items).isotope('layout').delay(550).remove()}
$.ajax({type:"POST",url:moodshop_ajax_url.ajax_url,data:{action:'moodshop_basr_get_product_output',shortcode:sc,},success:function(response){$container.append(response).find('.products').isotope({layoutMode:'fitRows',});console.log(response);$container.removeClass('load-product');var $banner=$container.find('.moodshop-banner');if($banner.next().length>0){$banner.css('min-height',$banner.next().outerHeight()+'px')}else if($banner.prev().length>0){$banner.css('min-height',$banner.prev().outerHeight()+'px')}},error:function(errorThrown){console.log(errorThrown)}})})}}}
$(document).ready(function(){BASR.animated();BASR.isotope();$('.moodshop-video-banner .wpb_wrapper').on('click',function(){console.log('click video');var video=$(this);$(this).find('iframe').each(function(){$(this).attr('src',$(this).attr('src')+'&autoplay=1')});setTimeout(function(){video.addClass('active')},300)});if($().slick){$('.wc-listing-filter i').on('click',function(){var $slick=$(this).closest('.wc-listing-filter');$(this).next().trigger('click');if($(this).closest('.slick-slide').next().hasClass('slick-center')){$slick.slick('slickNext')}else{$slick.slick('slickPrev')}});if($('.basr-home_slider').length>0){$('.home-left-slider .fake-next').on('click',function(){$(this).parent().next().find('.slick-next').trigger('click');$(this).parent().parent().next().find('.slick-prev').trigger('click')});$('.home-left-slider .fake-prev').on('click',function(){$(this).parent().next().find('.slick-prev').trigger('click');$(this).parent().parent().next().find('.slick-next').trigger('click')});$('.home-right-slider .fake-next').on('click',function(){$(this).parent().next().find('.slick-prev').trigger('click');$(this).parent().parent().prev().find('.slick-next').trigger('click')});$('.home-right-slider .fake-prev').on('click',function(){$(this).parent().next().find('.slick-next').trigger('click');$(this).parent().parent().prev().find('.slick-prev').trigger('click')});$('.basr-home_slider .fake-nav span').on('click',function(){$(this).closest('.basr-home_slider').toggleClass('toggle')});$('.basr-home_slider .fake-nav .fake-prev').on('click',function(){var $second=$(this).closest('.basr-home_slider').find('.home-slide-index .second'),$first=$(this).closest('.basr-home_slider').find('.home-slide-index .first'),first=$first.attr('data-index'),second=$second.attr('data-index'),total=$second.parent().attr('data-total');if($second.attr('data-index')==1){if($first.attr('data-index')==0){second=total%10;$second.attr('data-index',second);first=parseInt(total/10)
$first.attr('data-index',first)}
else{second=$second.attr('data-index')-1;$second.attr('data-index',second)}}else if($second.attr('data-index')==0){second=$second.attr('data-index')-1;$second.attr('data-index',second);first=$first.attr('data-index')-1;$second.attr('data-index',first)}else{second=$second.attr('data-index')-1;$second.attr('data-index',second)}
$first.html(first);$second.html(second)})
$('.basr-home_slider .fake-nav .fake-next').on('click',function(){var $second=$(this).closest('.basr-home_slider').find('.home-slide-index .second'),$first=$(this).closest('.basr-home_slider').find('.home-slide-index .first'),first=$first.attr('data-index'),second=$second.attr('data-index'),total=$second.parent().attr('data-total');if($second.attr('data-index')==(total%10)&&$first.attr('data-index')==parseInt(total/10)){first=0;second=1;$second.attr('data-index',1);$first.attr('data-index',0)}else{if($second.attr('data-index')==9){second=0;$second.attr('data-index',second);first=parseInt($first.attr('data-index'))+1;$first.attr('data-index',first)}else{second=parseInt($second.attr('data-index'))+1;$second.attr('data-index',second)}}
$first.html(first);$second.html(second)})}
$('.basr-slick-wrapper').each(function(){$(this).slick()});$('.wrap-slick-slider').each(function(){var $wrap_slick=$(this);var slick_id=$(this).attr('data-uniqid')
var $main_slick=$(this).find(slick_id);$(this).find('.basr-slick_slider').each(function(){var data=jQuery.parseJSON($(this).attr('data-slider'));var args={slidesToShow:parseInt(data.slidesToShow),slidesToScroll:parseInt(data.slidesToScroll),arrows:(data.arrows=='true')?!0:!1,dots:(data.dots=='true')?!0:!1,infinite:(data.infinite=='true')?!0:!1,autoplay:(data.autoplay=='true')?!0:!1,vertical:(data.vertical=='true')?!0:!1,verticalSwiping:(data.vertical=='true')?!0:!1,adaptiveHeight:!0,prevArrow:'<i class="zmdi zmdi-chevron-left slick-prev"></i>',nextArrow:'<i class="zmdi zmdi-chevron-right slick-next"></i>',responsive:[{breakpoint:1024,settings:{slidesToShow:parseInt(data.responsive[0].settings.slidesToShow),slidesToScroll:1,}},{breakpoint:768,settings:{slidesToShow:parseInt(data.responsive[1].settings.slidesToShow),slidesToScroll:1,}},{breakpoint:668,settings:{slidesToShow:parseInt(data.responsive[2].settings.slidesToShow),slidesToScroll:1,}}],};if(data.asNavFor&&$(this).hasClass('slick-syn-nav')){args.asNavFor='.'+data.asNavFor;args.centerMode=!0;args.vertical=!0;args.verticalSwiping=!0;args.centerPadding=0}
if(data.asNavFor&&!$(this).hasClass('slick-syn-nav')){args.asNavFor='.'+data.asNavFor}
console.log(args);$(this).slick(args);$(this).closest('.wrap-slick-slider').find('.fake-prev').on('click',function(){$(this).closest('.wrap-slick-slider').find('.slick-prev').trigger('click')});$(this).closest('.wrap-slick-slider').find('.fake-next').on('click',function(){$(this).closest('.wrap-slick-slider').find('.slick-next').trigger('click')})});var height=$main_slick.outerHeight()/3;$wrap_slick.find('.slick-syn-nav .slick-slide').css('height',height+'px');$(window).resize(function(){var height=$main_slick.outerHeight()/3;$wrap_slick.find('.slick-syn-nav .slick-slide').css('height',height+'px')});$('.slick-syn-nav .slick-slide').on('click',function(){var index=$(this).attr('data-slick-index');$(this).closest('.slick-syn-nav').slick('slickGoTo',parseInt(index))})})}});$('.basr-progress').each(function(){var $this=$(this);var delay=$this.data('animation-delay');$this.one('inview',function(event,isInView,visiblePartX,visiblePartY){setTimeout(function(){$this.find('.progress-item').each(function(){var value=$(this).find('progress').attr('value');$(this).find('.tooltip').css('left','0');$(this).find('.tooltip').animate({left:value+'%'},3000);$(this).find('progress').prop('Counter',0).animate({Counter:value},{duration:3000,easing:'swing',step:function(now){$(this).attr('value',Math.ceil(now))}});$(this).find('.tooltip-inner span').prop('Counter',0).animate({Counter:value},{duration:3000,easing:'swing',step:function(now){$(this).text(Math.ceil(now))}})})},delay)})})})(jQuery)
;